...
Rails.application.configure do
  ...
  # Store uploaded files on local disk (note: files may not persist across deploys on Render free tier)
  # To use AWS S3 instead, change to :amazon and set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY env vars
  ...
  # Using :async for now (runs jobs in memory). To use SolidQueue, uncomment below and run:
  ...
  # ActionCable configuration for WebSockets (required for Turbo Streams broadcasts)
  config.action_cable.allowed_request_origins = [
    "https://#{Jumpstart.config.domain}",
    /https:\/\/.*\.onrender\.com/
  ]
