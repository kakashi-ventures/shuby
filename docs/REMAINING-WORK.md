...
- [x] **ruby_native 0.9.0 → 0.10.2 upgrade** (2026-06-04) — gem bump (`~> 0.10`); iOS universal links via new `ios:` block in `config/ruby_native.yml` (`bundle_id` + `team_id`, AASA auto-served at `/.well-known/apple-app-site-association`, verified 200 with `9R33QX48J7.app.shuby.rubynative`); App Store review prompt via `native_review_tag` shown once after a measurement save (flash flag in `MeasurementsController#create`, gated native-only in `application.html.erb`, +2 tests); `app.name` intentionally NOT added (0.10.2 no longer reads it — app name lives in the Ruby Native dashboard); UPSTREAM-ISSUES workarounds re-verified against the 0.10.2 source (both OPEN ones retained). No new test failures (36F/6E pre-existing on HEAD).
- [ ] **iOS binary rebuild & redeploy** against ruby_native 0.10.2 via `ruby_native deploy` — REQUIRED operational follow-up to the upgrade above (token-holder runs it). Needed because: 0.10.2 shows a config-error screen on gem/native version mismatch; the associated-domains entitlement for universal links is baked into the build; `native_review_tag` is a ≥0.9.3 bridge feature inert on older binaries.
- [ ] **Verify on device (post-deploy)**: tapping an `app.shuby.app` link opens the app (not Safari); the App Store review prompt surfaces after saving a measurement; safe-area insets / custom headers render cleanly (no double-inset regression).
